...
MySQL查询的生命周期：<br>
<br>
![avatar](/image/index/MySQL查询生命周期.png)
...
![avatar](/image/index/执行计划（1）.png)

扫描了153492条数据

模拟下实际过程中完整的业务流程，查询性能怎么样

![avatar](/image/index/查询耗时.png)

可以看出来，越往后面的查询性能瓶颈越大，耗时越高，我们看下最后一次查询的执行计划

![avatar](/image/index/最后查询执行计划.png)

也是扫描了153492条数据

如果数据量再进一步增加，查询的瓶颈也就会越大，最后一些业务数据处理会越来越慢，对于这样的业务场景，我们该如何去优化查询呢？

分析下原因，为什么最后一次查询跟第一次查询性能相差百倍，结合业务场景，随着一批一批数据处理，status状态被更新成2，到最后一些数据的时候，前置已经处理完的数据还是被MySQL服务器计算，筛选到status=1的数据，然后选取100条返回。因此优化的方式在于如何减少后面查询扫描的数据行数？

我们假设，如果建立curr_date和status的联合索引，可不可以解决这个问题？

![avatar](image/index/重建索引.png)

这个并不是最佳的优化手段，而且status字段区分度很低，并且会更新操作，引起索引重建，存储空间也会相应增大，得不偿失。
<br>
...
![avatar](/image/index/curr_date索引展示.png)

对比下，index on ('curr_date', 'status'):

![avatar](/image/index/重建索引列展示.png)

后者的Cardinality比前者更小，所以综合考虑联合索引就建立在curr_date列上。
...
FROM
    presell_market.coupon_remind
WHERE
    curr_date = '20181113' AND status = 1 AND is_valid = 1 AND id > #{beginId# LIMIT 100;
```

我们来模拟下效果，完整的一次业务查询：

![avatar](/image/index/查改查询耗时.png)

![avatar](/image/index/查询耗时.png)

跟最初的查询对比，查询性能提升了很多，让我们看下查改之后的第一次执行计划：

![avatar](/image/index/查改第一次查询执行计划.png)
...
![avatar](/image/index/查改最后一次查询计划.png)
