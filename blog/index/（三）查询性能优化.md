...
- type：访问类型，表示MySQL在表中找到所需行的方式：
 ...
 11. index：跟ALL相同，当索引树被遍历时为index，通常比ALL高效，因为只遍历索引，会比table的数据小，此时Extra列的值为Using Index。或者从索引树开始做全表扫描，join类型也是index，只不过此时Extra列不出现Using Index
...
<br>**改造后**<br>
![改造后](/image/index/首次查询执行计划（查改）.png)
